...
public:
	...
	void enter_data()
	{
		...
		int gate_num = read_int("How many gates are there? ", 1, 20);
	...
	void show_stats()
	{
		...
		double avg_land_queue_time = landing_queue->get_total_wait() / landing_queue->get_num_served();
		std::cout << "The average wait time for the landing queue: " << avg_land_queue_time << std::endl;
		...
		double avg_depart_queue_time = departure_queue->get_total_wait() / departure_queue->get_num_served();
		std::cout << "The average wait time for the departure queue: " << avg_depart_queue_time << std::endl;
