	...
	void update(int clock)
	{
		...
		if (!the_queue.empty()) {
			...
			if ((clock - plane->start_service_time) > plane->service_time) {
				...
				//push the plane onto the tail of the departure queue
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				//pop the plane from the landing queue, will be put onto the service queue
				landing_queue->the_queue.pop();

				// FIXME: calculate the wait time of the plane in the landing queue
				int wait_time;
				wait_time = clock - plane->arrival_time;	//the current time minus the time it arrived at airport
